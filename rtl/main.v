...
`elsif	BKRAM_ACCESS
	localparam	RESET_ADDRESS = 92274688;
`endif
	//
	...
	localparam [31:0] GPSCLOCK_DEFAULT_STEP = 32'had371cd9;
...
`include "builddate.v"
 ...
 // 0x0000008
	assign	     rtcdate_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h3));
 // 0x000000c
 ...
 // 0x0000020 - 0x000002f
	assign	         rtc_sel = ((wb_dio_sel)&&((wb_addr[ 5: 3] &  3'h7) ==  3'h2));
 // 0x0000040 - 0x000004f
	assign	         gtb_sel = ((wb_dio_sel)&&((wb_addr[ 5: 3] &  3'h7) ==  3'h3));
 // 0x0000060 - 0x000007f
	assign	        netp_sel = ((wb_dio_sel)&&((wb_addr[ 5: 3] &  3'h7) ==  3'h4));
 // 0x0000080 - 0x000009f
	assign	    flashcfg_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h01); // 0x0800000
	assign	scope_sdcard_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h02); // 0x1000000 - 0x1000007
	assign	        gpsu_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h03); // 0x1800000 - 0x180000f
	assign	      sdcard_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h04); // 0x2000000 - 0x200000f
	assign	        uart_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h05); // 0x2800000 - 0x280000f
	assign	      wb_sio_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h06); // 0x3000000 - 0x300001f
//x2	Was a master bus as well
	assign	         cfg_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h07); // 0x3800000 - 0x380007f
	assign	        mdio_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h08); // 0x4000000 - 0x400007f
	assign	      wb_dio_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h09); // 0x4800000 - 0x48000ff
//x2	Was a master bus as well
	assign	        netb_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h0a); // 0x5000000 - 0x5003fff
	assign	       bkram_sel = ((wb_addr[25:21] &  5'h1f) ==  5'h0b); // 0x5800000 - 0x581ffff
...
//x2	Was a master bus as well
	...
	always	@(posedge i_clk)
		...
		casez( wb_addr[2:0] )
			...
			3'h3: r_wb_sio_data <= rtcdate_data;
	...
	always	@(posedge i_clk)
		casez({		clrled_ack,
				gck_ack,
				rtc_ack,
				gtb_ack	}) // netp default
			4'b1???: r_wb_dio_data <= clrled_data;
			4'b01??: r_wb_dio_data <= gck_data;
			4'b001?: r_wb_dio_data <= rtc_data;
			4'b0001: r_wb_dio_data <= gtb_data;
			default: r_wb_dio_data <= netp_data;
	...
	begin
		casez({		flashcfg_ack,
				...
				bkram_ack,
				flash_ack	})
			12'b1???????????: wb_idata <= flashcfg_data;
			12'b01??????????: wb_idata <= scope_sdcard_data;
			12'b001?????????: wb_idata <= gpsu_data;
			12'b0001????????: wb_idata <= sdcard_data;
			12'b00001???????: wb_idata <= uart_data;
			12'b000001??????: wb_idata <= wb_sio_data;
			12'b0000001?????: wb_idata <= cfg_data;
			12'b00000001????: wb_idata <= mdio_data;
			12'b000000001???: wb_idata <= wb_dio_data;
			12'b0000000001??: wb_idata <= netb_data;
			12'b00000000001?: wb_idata <= bkram_data;
			12'b000000000001: wb_idata <= flash_data;
