...
`endif
	...
	localparam	ZIP_ADDRESS_WIDTH = 19;	// Zip-CPU address width
	//
...
`include "builddate.v"
	...
	// Wishbone slave definitions for bus wb(SIO), slave clrled
 ...
 // 0x00010
	assign	        gpio_sel = ((wb_sio_sel)&&(wb_addr[ 4: 2] ==  3'h2));
 // 0x00020
	assign	    pwrcount_sel = ((wb_sio_sel)&&(wb_addr[ 4: 2] ==  3'h3));
 // 0x00030
	assign	     rtcdate_sel = ((wb_sio_sel)&&(wb_addr[ 4: 2] ==  3'h4));
 // 0x00040
	assign	        spio_sel = ((wb_sio_sel)&&(wb_addr[ 4: 2] ==  3'h5));
 // 0x00050
	assign	     version_sel = ((wb_sio_sel)&&(wb_addr[ 4: 2] ==  3'h6));
 // 0x00060
	assign	      clrled_sel = ((wb_sio_sel)&&(wb_addr[ 4: 2] ==  3'h7));
 // 0x00070 - 0x0007f
	assign	         gck_sel = ((wb_dio_sel)&&((wb_addr[ 3: 2] &  2'h3) ==  2'h0));
 // 0x00000 - 0x0000f
	assign	         rtc_sel = ((wb_dio_sel)&&((wb_addr[ 3: 2] &  2'h3) ==  2'h1));
 // 0x00010 - 0x0001f
	assign	         gtb_sel = ((wb_dio_sel)&&((wb_addr[ 3: 2] &  2'h2) ==  2'h2));
 // 0x00020 - 0x0003f
	assign	scope_sdcard_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h01); // 0x10000 - 0x10007
	assign	        gpsu_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h02); // 0x20000 - 0x2000f
	assign	      sdcard_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h03); // 0x30000 - 0x3000f
	assign	        uart_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h04); // 0x40000 - 0x4000f
	assign	      wb_dio_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h05); // 0x50000 - 0x5003f
//x2	Was a master bus as well
	assign	         cfg_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h06); // 0x60000 - 0x6007f
	assign	        mdio_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h07); // 0x70000 - 0x7007f
	assign	      wb_sio_sel = ((wb_addr[18:14] &  5'h1f) ==  5'h08); // 0x80000 - 0x8007f
//x2	Was a master bus as well
	assign	       bkram_sel = ((wb_addr[18:14] &  5'h10) ==  5'h10); // 0x100000 - 0x1fffff
	//
	...
	assign	     wbu_dwb_sel = ((wbu_addr[19:19] &  1'h1) ==  1'h0); // 0x00000 - 0x1fffff
//x2	Was a master bus as well
	assign	     zip_dbg_sel = ((wbu_addr[19:19] &  1'h1) ==  1'h1); // 0x200000 - 0x200007
	//
	...
	always	@(posedge i_clk)
		...
		casez( wb_addr[4:2] )
			...
			3'h6: r_wb_sio_data <= version_data;
			default: r_wb_sio_data <= clrled_data;
		endcase
	...
	always	@(posedge i_clk)
		casez({		gck_ack,
				rtc_ack	}) // gtb default
			2'b1?: r_wb_dio_data <= gck_data;
			2'b01: r_wb_dio_data <= rtc_data;
	...
	begin
		casez({		scope_sdcard_ack,
				...
				mdio_ack,
				wb_sio_ack	})
			8'b1???????: wb_idata <= scope_sdcard_data;
			8'b01??????: wb_idata <= gpsu_data;
			8'b001?????: wb_idata <= sdcard_data;
			8'b0001????: wb_idata <= uart_data;
			8'b00001???: wb_idata <= wb_dio_data;
			8'b000001??: wb_idata <= cfg_data;
			8'b0000001?: wb_idata <= mdio_data;
			8'b00000001: wb_idata <= wb_sio_data;
...
`ifdef	RTC_ACCESS
`ifdef	GPSTRK_ACCESS
	rtcgps	#(32'h2af31d) thertc(i_clk, i_reset,
...
`ifdef	WBUBUS_MASTER
	localparam	BUSUART_BITS=$clog2(BUSUART);
	// The Host USB interface, to be used by the WB-UART bus
	rxuartlite	#(BUSUART_BITS[4:0],BUSUART[$clog2(BUSUART)-1:0])
		rcv(i_clk, i_wbu_uart_rx,
				wbu_rx_stb, wbu_rx_data);
	txuartlite	#(BUSUART_BITS[4:0],BUSUART[$clog2(BUSUART)-1:0])
		txv(i_clk,
				wbu_tx_stb,
				wbu_tx_data,
				o_wbu_uart_tx,
...
`ifdef	INCLUDE_ZIPCPU
	...
	wbpriarbiter #(32,19)	bus_arbiter(i_clk,
...
`ifdef	BUS_DELAY_NEEDED
	busdelay #(19)	wbu_dwbi_delay(i_clk, i_reset,
		wbu_dwbi_cyc, wbu_dwbi_stb, wbu_dwbi_we, wbu_dwbi_addr, wbu_dwbi_odata, wbu_dwbi_sel,
...
`endif
	clrled clrled0(i_clk,
		...
		wb_data, clrled_counter, clrled_data,
