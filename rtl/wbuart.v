...
module	wbuart(i_clk, i_rst,
	...
	// thus.)
	wire	[(LGFLEN-1):0]	check_cutoff;
	assign	check_cutoff = -3;
	always @(posedge i_clk)
		o_cts = (rxf_status[(LGFLEN+1):2] > check_cutoff);
	...
	wire	handshaking = 1'b0;
	reg	q_rts, ck_rts, r_rts;
	always	@(posedge i_clk)
		q_rts <= i_rts;
	always	@(posedge i_clk)
		ck_rts <= q_rts;
	// On our last clock, create a signal we can use to signal the
	// transmitter to start
	always	@(posedge i_clk)
		r_rts <= (!tx_busy)&&((handshaking)?(ck_rts):1'b1);
